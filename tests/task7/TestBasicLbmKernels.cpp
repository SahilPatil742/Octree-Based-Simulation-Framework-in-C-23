...
void testInitialization1() {
  ...
  for (auto [i, cell] : std::views::enumerate(cells)) {
    ...
    for (auto q : std::views::iota(0uz, lbm::D3Q19::Q)) {
      pdfsExpected[cellIdx, q] = rhoView[cellIdx] * lbm::D3Q19::W[q]; // NOLINT(cppcoreguidelines-pro-bounds-constant-array-index)
    }
  }
...
void testMacroscopicQuantities1() {
  ...
  for (auto [i, cell] : std::views::enumerate(cells)) {
    ...
    for (auto q : std::views::iota(0uz, lbm::D3Q19::Q)) {
      pdfs[cellIdx, q] = rho * lbm::D3Q19::W[q]; // NOLINT(cppcoreguidelines-pro-bounds-constant-array-index)
    }
  }
